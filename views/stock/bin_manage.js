...
jQuery(function($) {
	...
	jQuery(grid_selector).jqGrid({
		...
		colModel:[
			...
			{name:'loc_name',index:'loc_name', width:90, sortable:true,editable: true,edittype:'select',editoptions:{value:getLocs()}} 
		], 
		...
		},		
		{
			...
			afterSubmit:function(result){
				...
				if (result&&jQuery.parseJSON(result.responseText).status) {
					...
					var para={'method':'getBins'};
					var grid_data=exeJson(para);
					jQuery(grid_selector).jqGrid('setGridParam',{data: grid_data})
					.trigger('reloadGrid');
					return true;					
				}
